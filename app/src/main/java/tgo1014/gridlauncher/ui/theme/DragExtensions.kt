...
internal suspend fun AwaitPointerEventScope.awaitConsumedVerticalPointerSlopOrCancellation(
    pointerId: PointerId,
 ...
 * @see detectHorizontalDragGestures
 */
suspend fun PointerInputScope.detectConsumedVerticalDragGestures(
    onDragStart: (Offset) -> Unit = { },
    onDragEnd: () -> Unit = { },
    onDragCancel: () -> Unit = { },
    ...
    awaitEachGesture {
        val down = awaitFirstDown(requireUnconsumed = false)
        var overSlop = 0f
        val drag = awaitConsumedVerticalPointerSlopOrCancellation(down.id, down.type) { change, over ->
            change.consume()
            overSlop = over
        }
        if (drag != null) {
...
internal suspend inline fun AwaitPointerEventScope.awaitPointerSlopOrCancellation(
    ...
    while (true) {
        val event = awaitPointerEvent()
        val dragEvent = event.changes.fastFirstOrNull { it.id == pointer } ?: return null
        if (dragEvent.isConsumed && ignoreConsumed) {
            return null
